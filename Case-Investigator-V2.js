// ==UserScript==
// @name         AgileBlue Case Investigator V2
...
(function () {
    ...
    // ========================================
    const CONFIG = {
        IPINFO_TOKEN: 'ce0a8bab925670',
        TIMEOUTS: {
            FORM_CHECK: 500,
            FORM_MAX_ATTEMPTS: 20,
            EMAIL_CHECK: 600,
            EMAIL_MAX_ATTEMPTS: 20,
            ALERT_CHECK: 500,
            ALERT_MAX_ATTEMPTS: 40,
            DEBOUNCE: 500,
            DROPDOWN_OPEN: 1000,
            SUBMIT_DELAY: 500,
        ...
        SELECTORS: {
            ALERT_TABLE: '[data-testid="alert-table-container"]',
            ALERT_ROWS: '[data-testid="alert-table-container"] .MuiTableBody-root .MuiTableRow-root',
            USERNAME: 'p.username',
            ASSIGNMENT_INPUT: 'input.MuiFilledInput-input',
            STATUS_DROPDOWN: '.MuiSelect-root.MuiSelect-select',
            STATUS_LABEL: 'label[for="Status"]',
            NARRATIVE_TEXTAREA: 'textarea[name="Notes"]',
            SAVE_BUTTON: 'button[aria-label="Save"]',
            ADD_NARRATIVE_BUTTON: 'button.MuiButton-outlinedSecondary',
            CREATE_EMAIL_BUTTON: 'button.MuiButton-outlined[style*="color: rgb(255, 255, 255)"]',
            NEW_ALERTS_TOGGLE: 'input.MuiSwitch-input[type="checkbox"]',
            CLIENT_ID_INPUT: 'input[name="ClientId"]'
        },
        STATUS_VALUES: {
            INVESTIGATING: 'Investigating',
            CLOSED: 'Closed',
            WAITING_FOR_CLIENT: '13'
        },
        CLOSED_STATUSES: ['benign', 'malicious', 'waiting for client']
    };
    ...
    const domCache = {
        ...
        get(key, selector, context = document) {
            ...
            // Re-query if element was removed
            this.elements.delete(key);
            return this.get(key, selector, context);
        },
        clear(key) {
            if (key) {
                this.elements.delete(key);
    ...
    // Helper function to check if case is assigned to user
    function isAssignedToUser(abbreviatedName) {
        const assignmentInput = domCache.get('assignmentInput', CONFIG.SELECTORS.ASSIGNMENT_INPUT);
        return assignmentInput && assignmentInput.value === abbreviatedName;
    }
    ...
    function queryIPInfo(ipAddress) {
        return new Promise((resolve) => {
            ...
            GM_xmlhttpRequest({
                method: 'GET',
                url: `https://ipinfo.io/${encodeURIComponent(ipAddress)}/json?token=${CONFIG.IPINFO_TOKEN}`,
                headers: {
    ...
    function isCaseClosed() {
        const statusElement = domCache.get('statusDropdown', CONFIG.SELECTORS.STATUS_DROPDOWN);
        if (!statusElement) {
            console.log('Case status element not found. Selector:', CONFIG.SELECTORS.STATUS_DROPDOWN);
            return false;
        }
        const status = statusElement.textContent.trim().toLowerCase();
        console.log('Current case status:', status);
        return CONFIG.CLOSED_STATUSES.includes(status);
    }

    // Helper function to remove resolution buttons
    function removeResolutionButtons() {
        const escalateButton = document.getElementById('escalateButton');
        const closeBenignButton = document.getElementById('closeBenignButton');
    ...
    // Helper function to get resolution for client
    function getResolutionForClient(rule, clientId, resolutionType) {
        const resolutions = rule.resolutions[resolutionType];
        if (!resolutions) return null;
        
        // Find client-specific resolution
        const clientResolution = resolutions.find((c) =>
            Array.isArray(c.client_id) ? c.client_id.includes(clientId) : c.client_id === clientId
        );
        
        // Fall back to default
        return clientResolution || resolutions.find((c) => c.client_id === 'default');
    }
    ...
    function addButtons(abbreviatedName) {
        ...
        if (isCaseClosed()) {
            console.log('Case is closed, removing resolution buttons and skipping addition.');
            removeResolutionButtons();
            return false;
        }

        // Find all alert rows
        const rows = document.querySelectorAll(CONFIG.SELECTORS.ALERT_ROWS);
        if (!rows.length) {
